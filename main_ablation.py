...
# === 均值-方差效用持久化配置文件路径 ===
PROGRESS_LOG_FILE = "aaai_experiment_progress.csv"  # 被下方seed逻辑覆盖
...
class HFMMEnvironment:
    ...
    def step(self, action_mm, action_it, adv_decay=1.0):
        ...
        if 'a_share' in self.dataset_type:
            ...
            carrying_cost = -0.000001 * abs(self.inventory)
        else:
        ...
        if 'binance' in self.dataset_type:
            r_wealth = (raw_r_wealth / (prev_portfolio_value + 1e-12)) * 5000000.0
        elif 'fi2010' in self.dataset_type:
            r_wealth = (raw_r_wealth / (prev_portfolio_value + 1e-12)) * 10000.0
        else:
...
def main():
    ...
    all_modes = ["our", "as", "ho_stoll", "ppo", "sac", "hrl", "mappo", "deeplob", "timesfm", "moment", "timer_xl", "ablation_no_blend", "ablation_no_dib", "ablation_no_it"]
            ...
            for mode in modes_to_test:
                ...
                for SEED_VAL in seed_list:
                    ...
                    try:
                        ...
                        if mode in ("our", "ablation_no_blend", "ablation_no_dib", "ablation_no_it"):
                        ...
                        elif mode in ["ablation_no_dib", "ablation_no_it", "ablation_no_blend"]:
                            agent = StabilizedAdversarialMASAC(
                                state_dim=32,
                                action_dim_mm=2,
                                action_dim_it=1,
                            ...
                            while not val_done:
                                ...
                                with torch.no_grad():
                                    ...
                                    if mode in ("our", "ablation_no_blend", "ablation_no_dib", "ablation_no_it"):
                                        ...
                                        elif LOADER_TYPE in ['lob_bench', 'trades_lob', 'a_share', 'fi2010']:
                                            action_mm_val = torch.clamp(torch.abs(action_mm_raw[..., :2]) * config['action_scale'] * 2.0, 0.01, 3.0)
                                        else:
